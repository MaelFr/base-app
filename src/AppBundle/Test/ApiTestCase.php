...
class ApiTestCase extends KernelTestCase
{
    ...
    {
        $birthDate = new \DateTime(date("Y-m-d",strtotime("1980-01-01 + ".rand(0,round((strtotime("2010-12-31") - strtotime("1980-01-01")) / (60 * 60 * 24)))." days")));
        $data = array_merge([
            'birthDate' => $birthDate,
            'createdBy' => $this->getEntityManager()->getRepository('AppBundle:User')->findAny(),
        ], $data);
        $accessor = PropertyAccess::createPropertyAccessor();
        $member = new Member();
        foreach ($data as $key => $value) {
            $accessor->setValue($member, $key, $value);
        }
